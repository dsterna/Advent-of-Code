...
const App: React.FC = () => {
  const getCoord = (latestPlace: ICord, steps: string): ICord[] => {
    let direction: string = steps.charAt(0);
    let numerOfSteps: number = parseInt((steps.substring(1, steps.length)));
    let lastEndPoint = latestPlace;
    let tempSteps = lastEndPoint.steps;
    ...
    switch (direction) {
      case "U":
        for (let index = 1; index < numerOfSteps + 1; index++) {
          x = lastEndPoint.x;
          y = lastEndPoint.y + index;
          tempArray.push({ x: x, y: y, steps: tempSteps += 1 })
        }
        break;
      case "D":
        for (let index = 1; index < numerOfSteps + 1; index++) {
  ...
  let firstTrack: ICord[] = [{ x: 0, y: 0, steps: 0 }]
  for (let index = 0; index < firstWire.length; index++) {
    firstTrack = [...firstTrack, ...(getCoord(firstTrack[firstTrack.length - 1], firstWire[index]))];
  }

  let secondWire: string[] = second.split(",")
  let secondTrack: ICord[] = [{ x: 0, y: 0, steps: 0 }]

  for (let index = 0; index < secondWire.length; index++) {
    secondTrack = [...secondTrack, ...(getCoord(secondTrack[secondTrack.length - 1], secondWire[index]))];
  }


  let ary1 = firstTrack.map(elem => elem.x.toString() + " " + elem.y.toString())
  let ary2 = secondTrack.map(elem => elem.x.toString() + " " + elem.y.toString())

  let hitArray1: ICord[] = [];
  ary1.forEach((elem, index) => {
    if (ary2.includes(elem)) {
      hitArray1.push(firstTrack[index])
      return;
    }
  }
  )
  ...
  let hitArray2: ICord[] = [];
  ary2.forEach((elem, index) => {
    if (ary1.includes(elem)) {
      hitArray2.push(secondTrack[index])
      return;
    }
  }
  )
